...
uncles(X) :- father(Y,X),brothers(Y). % Выводит на экран всех родных братьев папы X 
uncles(X) :- mother(Y,X),brothers(Y). % Выводит на экран всех родных братьев мамы X 

% Task 13:
aunt(X,Y) :- parent(Z,Y),sister(X,Z). % Проверяет, является ли X тетей Y
aunts(X) :- father(Y,X),sisters(Y). % Выводит на экран всех родных сестер папы X 
aunts(X) :- mother(Y,X),sisters(Y). % Выводит на экран всех родных сестер мамы X 
